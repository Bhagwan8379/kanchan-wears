...
const Carousel = () => {
    ...
    return (
        <div className="relative w-auto mt-16 h-50 sm:w-full sm:h-[500px] overflow-hidden transition-colors duration-1000">
            {/* Carousel items */}
            <div div className="relative h-full  w-full" >
                {
                    carouselItems.map((item, index) => (
                        <div
                            key={item.id}
                            className={`absolute inset-0 transition-opacity duration-1000 flex items-center ${index === currentIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
                        >
                            <img
                                src={item}
                                alt={index}
            ...
            < button
                onClick={handlePrev}
                className="absolute cursor-pointer left-4 top-1/2 z-20 -translate-y-1/2 bg-white/30 hover:bg-pink-400 text-white p-3 rounded-full sm:backdrop-blur-sm transition-all duration-300 hover:scale-110"
            >
            ...
            <button
                onClick={handleNext}
                className="absolute cursor-pointer right-4 top-1/2 z-20 -translate-y-1/2 bg-white/30 hover:bg-pink-400 text-white p-3 rounded-full sm:backdrop-blur-sm transition-all duration-300 hover:scale-110"
            >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="absolute bottom-6 left-0 right-0 z-20 flex justify-center gap-2">
                {carouselItems.map((_, index) => (
                    <button
                        ...
                        className={`sm:h-2 h-1 w-2 sm:w-8 rounded-full transition-all duration-300 ${index === currentIndex ? 'bg-white w-12' : 'bg-white/50'}`}
                    />
                ))}
            </div>
        </div >
            ...
            <div className="max-w-6xl mx-auto">
                <div className="flex flex-col md:flex-row items-center gap-12">
                    ...
                    <div className="w-full md:w-1/2 text-left">
                        ...
                        <button
                            className="bg-rose-500 hover:bg-rose-700 text-white font-medium md:py-3.5 md:px-8 py-2 px-6 rounded-full flex items-center gap-2 shadow-md transition-transform duration-200 hover:scale-105 active:scale-95"
                        >
